...
將 `chrome-settings-collector.ps1` 的預設匯出行為改成：當使用者加上 `-Export` 但**沒有**指定 `-Output` 時，輸出目錄預設建立在**腳本所在目錄**，而不是目前工作目錄。
...
- [x] 驗證匯出結果，因為要確認 `-Export` 在未指定 `-Output` 時，真的會輸出到腳本同目錄，而且既有指定路徑行為不受影響。

## 預期行為

- `.\chrome-settings-collector.ps1 -Export`
  - 預設輸出到 `D:\check_chrome\chrome-settings-export-YYYYMMDD-HHMMSS`
- `.\chrome-settings-collector.ps1 -Export -Output D:\check_chrome\export`
  - 仍輸出到使用者指定目錄
